...
if(isset($valider)){
    if(!preg_match("#^[a-zA-zéèêïÉÈÊÏ \-]+$#",$nom))
        $message="<div class='err'>Nom invalide!</div>";
    if(!preg_match("#^[a-zA-zéèêïÉÈÊÏ \-]+$#",$prenom))
        $message.="<div class='err'>Prénom invalide!</div>";
    if (!filter_var($email, FILTER_VALIDATE_EMAIL))
        $message.="<div class='err'>Email invalide!</div>";
    if($mdp!=$rmdp)
        $message.="<div class='err'>Mot de passes non identiques!</div>";
    if($message === ""){
